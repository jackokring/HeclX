...
public class MathCmds extends org.hecl.Operator {
	...
	switch(cmdcode) {
	  ...
	  case ABS:
 	  ...
 	  case SQRT:
 	    return DoubleThing.create(Mith.sqrt(a.doubleValue()));
 	  case FLOOR:
	  ...
	  case NOT:
 	  ...
 	  case LOG:
 	    return DoubleThing.create(Mith.log(a.doubleValue()));
 	  case ASIN:
 	    return DoubleThing.create(Mith.asin(a.doubleValue()));
 	  case ACOS:
 	    return DoubleThing.create(Mith.acos(a.doubleValue()));
 	  case ATAN:
 	    return DoubleThing.create(Mith.atan(a.doubleValue()));
 	  case EXP:
 	    return DoubleThing.create(Mith.exp(a.doubleValue()));
	}
	...
	switch(cmdcode) {
	  ...
	  case OR:
		...
		return IntThing.create(a.intValue() | b.intValue());
 	    ...
 	    throw new HeclException("Integral argument required.");
 	  case POW:
 	    return DoubleThing.create(Mith.pow(a.doubleValue(), b.doubleValue()));
	  case EQ:
	...
	switch(cmdcode) {
	  ...
	  case MINUS:
            num = IntThing.ZERO;
            if(argv.length==2) return new Thing((NumberThing)binary(BINSUB,ip, num,
                                         NumberThing.asNumber(argv[1])).getVal());
	    for(int i=1; i<argv.length; i+=2) {
		num = (NumberThing)binary(BINADD, ip, num,
					  NumberThing.asNumber(argv[i])).getVal();
                if(i+1<argv.length) num = (NumberThing)binary(BINSUB, ip, num,
					  NumberThing.asNumber(argv[i+1])).getVal();
	    }
            return new Thing(num);
          case DIV:
            num = IntThing.ONE;
            if(argv.length==2) return new Thing((NumberThing)binary(BINDIV,ip, num,
                                         NumberThing.asNumber(argv[1])).getVal());
	    for(int i=1; i<argv.length; i+=2) {
		num = (NumberThing)binary(BINMUL, ip, num,
					  NumberThing.asNumber(argv[i])).getVal();
                if(i+1<argv.length) num = (NumberThing)binary(BINDIV, ip, num,
					  NumberThing.asNumber(argv[i+1])).getVal();
	    }
            return new Thing(num);
	  case MUL:
	...
	cmdtable.put("/", new MathCmds(DIV,-1,-1));
	...
	cmdtable.put(">=",new MathCmds(GE,2,2));
 	...
 	cmdtable.put("random",new MathCmds(RANDOM,0,0));
        cmdtable.put("^", new MathCmds(POW,2,2));
